...
const nodeAddressFromKey = (key) => {
  ...
  console.assert(
    Buffer.isBuffer(fullKeyBuffer),
    `nodeAddressFromKey: fullKeyBuffer is not a Buffer`
  )
...
const hashFromAddresses = (address1, address2) => {
  console.log(
    `hashFromAddreses: ${JSON.stringify(address1)}:${JSON.stringify(address2)}`
  )
  const hash = keccak256(
    Buffer.concat([Buffer.from(address1, `hex`), Buffer.from(address2, `hex`)])
  )
...
const sign = (data, key) => {
  ...
  console.assert(
    ...
    `sign: privateKey is not a Buffer`
  )
...
contract(`XyoStaked`, ([contractCreator, owner1]) => {
  ...
  const hashedAddresses = hashFromAddresses(
    owner1.slice(2),
    nodeAddress.toString(`hex`)
  )
  ...
  it(`create contracts`, async () => {
    token = await XyERC20Token.new(TW(10000000), `XY Token`, `XYO`, {
    ...
    xyoStake = await XyoStaked.new(token.address, 1, {
      from: contractCreator,
      gasPrice: 0
    })
  ...
  it(`approve tokens for staking`, async () => {
    const { logs } = await token.approve(xyoStake.address, TW(10), {
      from: owner1
    })
    console.log(`logs: ${JSON.stringify(logs)}`)
  ...
  it(`should stake a new node`, async () => {
    const { logs } = await xyoStake.stake(nodeAddressNumber, TW(10), {
      from: owner1
    })
    console.log(`logs: ${JSON.stringify(logs)}`)
  })
  ...
  it(`should have nothing available to withdraw`, async () => {
    ...
    assert.equal(
      parseInt(result, 10),
      0,
      `result(${parseInt(result, 10)}) should = 0`
    )
  })
  ...
  it(`should have stake request`, async () => {
    ...
    assert.equal(
      BN(request[0]).toString(16, 20),
      nodeAddress.toString(`hex`),
      `request.node(${BN(request[0]).toString(
        16,
        20
      )}) should = nodeAddress(${nodeAddress.toString(`hex`)})`
    )
    assert.equal(
      request[1].toString(10),
      TW(10).toString(10),
      `request.amount(${request[1].toString(10)}) should = TW(10)(${TW(
        10
      ).toString(10)})`
    )
  })
  ...
  it(`should have stake`, async () => {
    ...
    assert.equal(
      BN(node[1]).toString(16),
      nodeAddress.toString(`hex`),
      `node(${BN(node[1]).toString(
        16
      )}) should = nodeAddress(${nodeAddress.toString(`hex`)})`
    )
    assert.equal(
      BN(node[0]).toString(16),
      BN(owner1).toString(16, 20),
      `owner(${BN(node[0]).toString(16)}) should = owner1(${BN(owner1).toString(
        16,
        20
      )})`
    )
